...
if [ -z "$readFile1" ] && [ -z "$readFile2" ] && [ -z "$readFileS" ] && [ -z "$readFileP" ] ; then
   ...
   if [ "$constructGraph" = "Y" ] ; then
      mpirun -np $numProcs ${exePath}/buildG-MPI -pe 1 ${readFileP} -f $outGraphPrefix -p ${asmParaFileP} -t ${numThreads} -m ${maxMem}  > ${logFile}
   fi
   ...
   if [ "$constructGraph" = "Y" ] ; then
      mpirun -np $numProcs ${exePath}/buildG-MPI -se 1 ${readFileS} -f $outGraphPrefix -p ${asmParaFileP} -t ${numThreads} -m ${maxMem}  &> ${logFile}
   fi
   if [ "$simplifyGraph" = "Y" ] ; then
   ...
   if [ "$constructGraph" = "Y" ] ; then
      mpirun -np $numProcs ${exePath}/buildG-MPI -pe 2 ${readFile1} ${readFile2} -f $outGraphPrefix -p ${asmParaFileP} -t ${numThreads} -m ${maxMem}  &> ${logFile}
   fi
   if [ "$simplifyGraph" = "Y" ] ; then
   ...
   if [ "$constructGraph" = "Y" ] ; then
      mpirun -np $numProcs ${exePath}/buildG-MPI -pe 2 ${readFile1} ${readFile2} -se 1 ${readFileS} -f $outGraphPrefix -p ${asmParaFileP} -t ${numThreads} -m ${maxMem}  &> ${logFile}
   fi
   if [ "$simplifyGraph" = "Y" ] ; then
